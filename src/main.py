...
import config
import distributed
from data.utils import DataReader, SharedMemoryDataReader, get_dataset
...
def main(args, parser):
    ...
    if distributed_backend.is_master_process() and args.wandb:
        ...
        wandb.define_metric("model_norm/*", step_metric="iter")
        ...
        wandb.define_metric("effective_lr", step_metric="iter")
    ...
    if args.spectral_post_process != "none":
        ...
        opt = SPECTRA(
            ...
            clip_c=args.spectral_clip_c,
            ns_steps=args.spectral_ns_steps,
            apply_to=args.spectral_apply_to,
            warmup_steps=args.warmup_steps,
            base_lr=args.lr,
    ...
    if args.scheduler != "none":
        ...
        # Pass base_optimizer to scheduler when SPECTRA is active, to prevent
        # older PyTorch LR schedulers from calling SPECTRA.step() internally,
        # which would double-increment _step_count and corrupt the decay schedule.
        sched_opt = opt.base_optimizer if hasattr(opt, "base_optimizer") else opt
        if args.scheduler in ["cos", "linear"]:
            ...
            scheduler = (
                torch.optim.lr_scheduler.LambdaLR(sched_opt, lambda_schedule)
                if args.opt != "muon"
                else CombinedScheduler(opt, args)
            )
    ...
    # Only rank 0 checks for existing checkpoint to avoid race conditions
    # Other ranks will get resume_from via the barrier below
    ...
    # Synchronize resume_from across all ranks
    ...
    if distributed_backend.get_world_size() > 1:
        ...
        args.resume_from = resume_from_list[0]
    ...
    if args.shared_memory:
        datareaders["train"].cleanup()
        datareaders["val"].cleanup()
...
def get_data_readers(args, verbose=True):
    ...
    if args.shared_memory:
        # Use shared memory: rank 0 loads data, others attach
        # Only works with file paths (not pre-loaded numpy arrays)
        train_path = data_srcs["train"]
        val_path = data_srcs["val"]
        if not isinstance(train_path, str):
            raise ValueError(
                "--shared_memory requires dataset to return file paths, "
                f"but got {type(train_path)}. Use datasets like finewebedu, slimpajama, etc."
            )

        train_reader = SharedMemoryDataReader(
        ...
        val_reader = SharedMemoryDataReader(
            data_path=val_path,
            batch_size=args.batch_size,
            sequence_length=args.sequence_length,
            shm_name="val_shm",
...
def get_exp_name(
    ...
    # Append additional ignore args from command line
    if hasattr(args, "ignore_args_more") and args.ignore_args_more:
        ignore_args = list(ignore_args) + args.ignore_args_more
    ...
    prefix = f"{args.batch_size}x{args.acc_steps}x{args.world_size}_" + prefix
